...
export function TooltipComponent({tittleUrl}) {
  const [tooltipPosition, setTooltipPosition] = useState({ top: 0, left: 0 });
  const [isTooltipVisible, setIsTooltipVisible] = useState(false);

  const handleMouseMove = (event) => {
    setTooltipPosition({
      top: event.clientY , // Ajusta la posición vertical
      left: event.clientX , // Ajusta la posición horizontal
    });
  };
  ...
  return (
    <div className='relative flex h-16 w-full rounded-lg p-2 bg-gray-500'>
      <div
        className='group flex justify-start items-start overflow-hidden p-4 max-w-xs text-right'
        onMouseMove={handleMouseMove}
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        <h3 className='text-sm text-white font-bold whitespace-nowrap overflow-hidden text-ellipsis'>
        {tittleUrl}
        </h3>
        {isTooltipVisible && (
          <span
      ...
      <div className='flex flex-col justify-center items-center flex-1'>
        <ul className='flex flex-row flex-1 gap-2'>
          <div className='flex justify-center items-center flex-col'>
            <li className='text-xs font-bold'>Api100</li>
            <li className='text-xs font-semibold'>20%</li>
          </div>
          <div className='flex justify-center items-center flex-col'>
            <li className='text-xs font-bold'>Getlinked</li>
            <li className='text-xs font-semibold'>20%</li>
          </div>
          <div className='flex justify-center items-center flex-col'>
            <li className='text-xs font-bold'>kamehamedia</li>
            <li className='text-xs font-semibold'>20%</li>
          </div>
        </ul>
      </div>
